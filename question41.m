...
cons = [
    ...
    P1'*(x_var(1, 1)*A + x_var(1,2)*B)*P1 <= 1 + 10^-6, 
    P2'*(x_var(1, 1)*A + x_var(1,2)*B)*P2 <= 1 + 10^-6, 
...
% fimplicit(@(x, y) C_val(1,1)*x.^2 + (C_val(1,2)+C_val(2,1))*x.*y + C_val(2,2)*y.^2 - 1, ...
%     [-2000 2000 -2000 2000], 'r', 'LineWidth', 1.5);

fimplicit(@(x, y) C_val(1,1)*x.^2 + (C_val(1,2)+C_val(2,1))*x.*y + C_val(2,2)*y.^2 - 1, ...
    [-10 10 -10 10], 'r', 'LineWidth', 1.5);
